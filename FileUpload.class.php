...
class FileUpload{
    ...
    public $mime_arr = array(
        "image/gif"  => "gif",
        "image/jpg"  => "jpg",
        "image/jpeg" => "jpeg",
        "image/png"  => "png"
    );//允许上传文件的类型
    private $maxsize = 2 * 1024 * 1024;//允许上传文件的最大值
    ...
    function uploadFile($fileField){
        ...
        $name     = $_FILES['file']['name'][$fileField];
        $tmp_name = $_FILES['file']['tmp_name'][$fileField];
        $size     = $_FILES['file']['size'][$fileField];
        $error    = $_FILES['file']['error'][$fileField];
//        $this->filetype    = $this->mime_arr[$_FILES[$fileField]['type']];
