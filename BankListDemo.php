...
$type = ['贷记卡','借记卡','预付费卡','准贷记卡'];
$mysqli = new mysqli('192.168.3.29','root','banklink');
$mysqli->select_db('banklink');
echo '<pre />';
$i=0;
$types = ['贷记卡'=>1,'借记卡'=>0,'预付费卡'=>2,'准贷记卡'=>3];

foreach($temp as $key => $v ){
//    var_dump($val);die;
        if($v[2] && in_array($v[2],$type)){
            $v[2] = $types[$v[2]];
            $sql="insert into b_card_identify values(null,'$key','{$v[0]}','{$v[2]}','{$v[1]}')";
        }elseif($v[3] && in_array($v[3],$type)){
            $v[3] = $types[$v[3]];
            $sql="insert into b_card_identify values(null,'$key','{$v[0]}','{$v[3]}','{$v[2]}')";
        }elseif($v[4] && in_array($v[4],$type)){
            $v[4] = $types[$v[4]];
            $sql="insert into b_card_identify values(null,'$key','{$v[0]}','{$v[4]}','{$v[3]}')";
        }
        $i++;
        echo $i.'-----';
        echo '<br />';
